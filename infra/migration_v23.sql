...
--
-- Todos os índices são CREATE INDEX CONCURRENTLY para zero downtime em produção.
-- Execute fora de transação explícita (CONCURRENTLY não pode rodar em bloco BEGIN).
-- ============================================================

-- ── device_events ─────────────────────────────────────────────────────────────

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_device_events_device_hash
    ON device_events (tenant_id, device_hash, player_id)
    WHERE device_hash IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_device_events_ip_hash
    ON device_events (tenant_id, ip_hash, player_id)
    WHERE ip_hash IS NOT NULL;

-- ── financial_transactions ────────────────────────────────────────────────────

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_financial_transactions_bank_account_hash
    ON financial_transactions (tenant_id, bank_account_hash, player_id)
    WHERE bank_account_hash IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_financial_transactions_payment_instrument
    ON financial_transactions (tenant_id, payment_instrument, player_id)
    WHERE payment_instrument IS NOT NULL;

-- ── feature_snapshots ─────────────────────────────────────────────────────────

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_feature_snapshots_tenant_created
    ON feature_snapshots (tenant_id, created_at);

-- ── ingest_errors ─────────────────────────────────────────────────────────────

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_ingest_errors_tenant_created
    ON ingest_errors (tenant_id, created_at);

-- ── report_packages ───────────────────────────────────────────────────────────

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_report_packages_tenant_status_created
    ON report_packages (tenant_id, status, created_at);

-- ── alerts — suporte precision label ─────────────────────────────────────────

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_alerts_tenant_label_created
