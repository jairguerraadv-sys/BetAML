-- BetAML migration v35
-- Compatibilidade produto-completo: schema usado por API/stream/rules engine.
-- Idempotente para bases criadas por init-db.sql + migrations SQL.

BEGIN;

-- Alerts: campos usados por fila operacional, export CSV e prioridade manual.
ALTER TABLE alerts
...
-- Feature store: stream_processor faz upsert por tenant/player/date.
...
-- Case timeline: aceitar os eventos gerados pela API e pelo rules_engine.
...
-- Player status: unificar status emitidos por API, KYC e stream_processor.
...
-- KYC/RG/status: suportar contrato canônico e endpoints manuais legados.
