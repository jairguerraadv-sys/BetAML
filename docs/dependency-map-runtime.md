...
5. [services/api/alert_processor.py](/workspaces/BetAML/services/api/alert_processor.py) permanece desativado por padrão; o caminho operacional ativo de materialização de alertas/casos é apenas [services/rules_engine/main.py](/workspaces/BetAML/services/rules_engine/main.py).
...
| [services/api/alert_processor.py](/workspaces/BetAML/services/api/alert_processor.py) | Startup hook legado (`start_alert_consumer`, no-op por padrão) | `scoring.alerts` | - | Autoridade operacional removida; `rules_engine` materializa `alerts`, `cases`, `case_events` e atualiza `players` |
...
| [services/api/routers/compound_rules.py](/workspaces/BetAML/services/api/routers/compound_rules.py) | `GET/POST/PUT/DELETE /rules/compound`, `GET/POST/DELETE /rules/macros` | - | - | `compound_rules`, `rule_macros` |
| [services/api/routers/player_lists.py](/workspaces/BetAML/services/api/routers/player_lists.py) | CRUD `/player-lists` + upload CSV | - | - | `player_lists`, `player_list_entries` |
| [services/api/routers/alerts.py](/workspaces/BetAML/services/api/routers/alerts.py) | `GET /alerts`, `GET /alerts/{id}`, `POST /alerts/{id}/triage|close|link-to-case|label` | - | - | `alerts`, `cases` (vínculo), evidências relacionadas |
| [services/api/routers/cases.py](/workspaces/BetAML/services/api/routers/cases.py) | CRUD operacional de casos + report package (`/cases/{id}/report-package*`) | - | - | `cases`, `case_events`, `report_packages`, relação com `alerts` |
| [services/api/routers/reports.py](/workspaces/BetAML/services/api/routers/reports.py) | `POST/GET /reports/monthly-summary`, `GET /reports/monthly-summary/csv` | - | - | Agrega `alerts`, `cases`, `report_packages`, `rule_definitions`, `players` |
| [services/api/routers/feature_store.py](/workspaces/BetAML/services/api/routers/feature_store.py) | `GET /feature-store/players/{id}/history|current`, `GET /feature-store/population-stats` | - | - | `feature_snapshots` + Redis online feature store |
| [services/api/routers/players.py](/workspaces/BetAML/services/api/routers/players.py) | `GET /players/{id}/feature-history`, charts e rede, LGPD erase/export | - | - | `players`, `financial_transactions`, `bets`, `alerts`, `cases`; ClickHouse `betaml.player_features_daily` |
| [services/ml_service/main.py](/workspaces/BetAML/services/ml_service/main.py) | `POST /score`, `POST /train`, `POST /score/ab`, `POST /score/shap`, `GET /models*` | - | - | Reads ClickHouse `betaml.player_features_daily`; Writes `model_registry`, `model_inference_logs` |
| [services/api/routers/ml.py](/workspaces/BetAML/services/api/routers/ml.py) | `GET /model-registry*`, `POST /model-registry/{id}/promote|challenger` | - | - | `model_registry`, métricas A/B, promoção champion/challenger |
| [services/api/routers/admin.py](/workspaces/BetAML/services/api/routers/admin.py) | tenant settings, users, flags, scoring config, api keys, maintenance mode | - | `ingest.jobs` (onboarding sample) | `tenants`, `users`, `api_keys`, `system_flags`, `scoring_configs`, `audit_logs` |
| [services/api/routers/notifications.py](/workspaces/BetAML/services/api/routers/notifications.py) | `GET /notifications`, read/all | - | - | `notifications` |
